              ...
              <tr key={index} >
                ...
                <td className="">
                  {product.offer_reprofile_risk_target === "1" && (
                    <span className="text-black dark:text-white">1 - Conservative</span>
                  )}
                  {product.offer_reprofile_risk_target === "2" && (
                    <span className="text-black dark:text-white">2 - Balanced</span>
                  )}
                  {product.offer_reprofile_risk_target === "3" && (
                    <span className="text-black dark:text-white">3 - Balanced</span>
                  )}
                  {product.offer_reprofile_risk_target === "4" && (
                    <span className="text-black dark:text-white">4 - Balanced</span>
                  )}
                  {product.offer_reprofile_risk_target === "5" && (
                    <span className="text-black dark:text-white">5 - Aggressive</span>
                  )}
                </td>
                <td className="flex justify-center items-center">
                  <Link
                    href={`/customer-details?customerID=${product.bp_number_wm_core}`}
                    className="text-black dark:text-white p-2 bg-[#01ACD2] rounded-md my-2 w-20 flex justify-center items-center gap-2 cursor-pointer"
                  >
                    Profil
                    <CiShare1 />
                  </Link>
