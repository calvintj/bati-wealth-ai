...
import { Download, ExternalLink, Pencil, CheckCircle2, Calendar } from "lucide-react";
...
export default function OwnedProductTable() {
  ...
  const [appointmentCreated, setAppointmentCreated] = useState<Set<string>>(new Set());
  ...
  const handleJanjiTemu = async (customerId: string, productName: string) => {
    try {
      ...
      // Mark as created
      setAppointmentCreated((prev) => new Set(prev).add(`${customerId}-${productName}`));
      ...
      toast({
        title: "Error",
        description: error.message || "Gagal membuat task. Silakan coba lagi.",
        variant: "destructive",
      });
    }
  };
                ...
                >
                  ...
                  <td className="p-2">
                    <div className="flex justify-center items-center">
                      {product.profit > 0 ? (
                        <button
                          onClick={() => handleAmbilProfit(product.id_nasabah, product.nama_produk)}
                          disabled={profitTaken.has(`${product.id_nasabah}-${product.nama_produk}`)}
                          className={`px-3 py-1 text-xs font-medium rounded-md transition-all ${
                            profitTaken.has(`${product.id_nasabah}-${product.nama_produk}`)
                        ...
                        <button
                          onClick={() => handleJanjiTemu(product.id_nasabah, product.nama_produk)}
                          disabled={appointmentCreated.has(`${product.id_nasabah}-${product.nama_produk}`)}
                          className={`px-3 py-1 text-xs font-medium rounded-md transition-all flex items-center gap-1 ${
                            appointmentCreated.has(`${product.id_nasabah}-${product.nama_produk}`)
